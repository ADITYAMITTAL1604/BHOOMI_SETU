...
## Part 1.5: Database Bootstrapping & Seeding (Supabase)

To populate your Supabase PostGIS database with the full multi-state projects, parcels, and statutory workflows, run from your terminal:

```bash
python backend/db/seed.py --reset --source demo
```

This will:
1. Initialize PostGIS and UUID extensions on Supabase.
2. Create all statutory tables automatically.
3. Seed all 12 projects, 2,400 parcels, stages, compensation, R&R, and demo users.
